function subnomdir = GAGenericDecoupledDev(params)
startTime = clock;
initcputime = cputime;
...
%                 end
            ...
            if params.evaluateAlways
              nj.sources = nj.G.rasterDev;
              nj.sources(nj.indexNewG) = nj.G.genome(nj.indexNewG);
              nj.indexNewG = 1:numel(nj.sources);
            else
              nj.sources = nj.G.genome;
            end
            nj.generation = cont;
            nj.rangeid    = rind;
            nj.job        = evaluarRemote(terclus, nj.sources(nj.indexNewG), params, nj.generation, nj.rangeid, nj.indexNewG);
            ...
            for k=1:ntasks
                if isempty(job.Tasks(k).ErrorMessage)
                    ...
                    if goon
                        ...
                        indexesab = indexes(limitsByW(k,1):limitsByW(k,2));
                        for zz=(1):numel(fieldsToFill) 
                          %we shift zz by 1 (output{zz+1}) since the first
                          %value is the global time spent
                          jobstructs{e}.G.(fnames{fieldsToFill(zz)})(indexesab,:) = output{zz+1};
                        end
...
for k=1:numel(toUpdate)
    ...
    areError = find(cellfun(@(x)not(isempty(x)) && isfield(x, 'isError'), jobsActive{ei}.G.rasterDev(jobsActive{ei}.indexNewG)));
    if not(isempty(areError))
      errs = jobsActive{ei}.G.rasterDev(jobsActive{ei}.indexNewG(areError)); %#ok<NASGU>
...
%       P.rasterDev(ranges(rpri,:)) = {[]};
%       P.rasterEv(ranges(rpri,:)) = {[]};
    end
...
%we must multiply by 1e5 since numbers in the range 0-1 will not give rise to diverse random sequences
rndSeedDevs = rand(size(G))*1e5;
rndSeedEvs  = rand(size(G))*1e5;
...
if isempty(terclus.jobMgr)
  ...
  limitsByW = [1 numel(G)];
  if ~params.plotting 
      returnIndividuals = false(size(G));
  else
      returnIndividuals = true(size(G)); %return all individuals
  end
  taskinfo.task = 1;
  [outputs{:}] = doEvaluation(params.multiEvFun, G, indexesG, rndSeedDevs, rndSeedEvs, returnIndividuals,params.evparams,taskinfo);
  newjob = struct('Tasks', {struct('OutputArguments', {outputs}, 'ErrorMessage', {''})}, ...
                  'State', {'finished'}, ...
                  'UserData', {{limitsByW, returnIndividuals}}, ...
                  'fakeJob', {true});
else
  newjob = sendRemoteJob(terclus, params, G, indexesG, rndSeedDevs, rndSeedEvs, taskinfo, nargs);
end
rand(params.randmethod, tempseed);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function newjob = sendRemoteJob(terclus, params, G, indexesG, rndSeedDevs, rndSeedEvs, taskinfo, nargs)
  ...
  for z=1:st
    ...
    try
      ...
      for k=1:numel(indsByW)
        if params.plotting 
            returnIndividuals = true(size(rndSeedDevs)); %return all individuals
        else
            returnIndividuals = false(size(rndSeedDevs));
        end
        taskinfo.ntask = k;
        a = limitsByW(k,1);
        b = limitsByW(k,2);
        createTask(newjob,@doEvaluation, nargs, {params.multiEvFun, G(a:b), indexesG(a:b), rndSeedDevs(a:b), rndSeedEvs(a:b), returnIndividuals(a:b), params.evparams, taskinfo});
      end
...
% Caught-error breakpoint was hit in distcomp.pbsscheduler.pSubmitJob at line 155. The error was:
% 
% Error using ==> distcomp.pbsscheduler.pSubmitJob at 155
% Error executing the PBS script command 'qsub'. The reason given is 
%  /opt/matlabr2008a/toolbox/distcomp/bin/util/shellWrapper.sh: line 8:  8015 Violación de segmento   "${@}"
% 
% 
% 155     error('distcomp:pbsscheduler:UnableToFindService', ...
% 
% Caught-error breakpoint was hit in distcomp.abstractjob.submit at line 51. The error was:
% 
% Error using ==> distcomp.pbsscheduler.pSubmitJob at 155
% Error executing the PBS script command 'qsub'. The reason given is 
%  /opt/matlabr2008a/toolbox/distcomp/bin/util/shellWrapper.sh: line 8:  8015 Violación de segmento   "${@}"
...
% 
% 
% Error in ==> GAGenericDecoupledDev>evaluarRemote at 524
%   submit(newjob);
% 
% Error in ==> GAGenericDecoupledDev at 152
%             nj.job        = evaluarRemote(terclus, nj.sources(nj.indexNewG), params, nj.generation, nj.rangeid, nj.indexNewG);
% 
% Error in ==> mainGeneric at 264
%         subnomdir = gafun(params); %#ok<NASGU>
% 
% Error in ==> molMotConf>@(varargin)mainGeneric(params,postparams,varargin{:}) at 194
% fun = @(varargin)mainGeneric(params, postparams, varargin{:});

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [timeSpent varargout] = doEvaluation(multiEvFun, varargin)
timeStart   = cputime;
[varargout{1:nargout-1}] = feval(multiEvFun, varargin{:});
...
res = struct('Generation', {generation}, 'rangeid', {rangeid}, 'indexes', {indexes}, 'indexNewG', {indexNewG}, 'rasterDevs', {G.rasterDev(indexNewG)}, 'rasterEvs', {G.rasterEv(indexNewG)}); %#ok<NASGU>
...
if isfield(params, 'initialPopulation')
  if isstruct(params.initialPopulation)
    ...
    if ~sameranges
      error('The ranges should be the same!!!!');
    end
    for k=1:numel(params.initialPopulation.oldparams.terclus.numRanges)
      ...
      if exist(fname, 'file')
        ...
        load(fname, 'res');
        if exist('res', 'var')
          if isfield(res.rasterDevs{1}, 'pos') %#ok<NODEF>
            P.rasterDev(res.indexes) = res.rasterDevs(1:end);
          end
          P.rasterEv(res.indexes) = res.rasterEvs(1:end);
          clear res;
        end
      end
    end
